...
model ADL_binary(event='1') = logtime NEURO housing age age*logtime housing*logtime NEURO*logtime / dist=binomial solution;
...
** omit the term with highest p-value --> logtime*housing**; 

proc glimmix data=MYDATA.ADLupdate ;
class NEURO housing ID;
model ADL_binary(event='1') = logtime NEURO housing age age*logtime NEURO*logtime / dist=binomial solution;
random intercept logtime / type=un subject=ID solution;
run;

**This model with age does not converge**;
**omit age*logtime**;
...
**still doesn't converge --> try centering age**;

proc means data=MYDATA.ADLupdate noprint;
   var age;
   output out=age_mean mean=mean_age;
run;

data MYDATA.ADLupdate;
   if _N_ = 1 then set age_mean;
   set MYDATA.ADLupdate;
   age_c = age - mean_age;
run;

proc glimmix data=MYDATA.ADLupdate;
class NEURO housing ID;
model ADL_binary(event='1') = logtime NEURO housing age_c NEURO*logtime / dist=binomial solution;
random intercept logtime / type=un subject=ID solution;
run;

**centering age with the mean doesn't work, we decide to omit age from the model**;
**next, we test the significance of the NEURO*logtime interaction**;
...
** try now model with only random intercept and again test the interaction, we don't expect a different outcome**;
...
do NEURO= 0 to 1 by 1;
   do subject=1 to 1000 by 1;
    	 b=sqrt(1.3693)*rannor(-1);
     		do logtime=-10 to 10 by 0.1;
    			if NEURO=0 then y=exp(-0.3139 + 1.5334 + b + 0.3818*logtime)/(1+exp(-0.3139 + 1.5334 + b + 0.3818*logtime));
               else y=exp(-0.3139 + b + 0.3818*logtime)/(1+exp(-0.3139 + b + 0.3818*logtime));
          output;
    end;
  end;
end;
